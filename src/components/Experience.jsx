...
const AnimatedStrip = ({ side = 'left', className, children }) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={className}
      style={side === 'right' ? { right: 'auto', left: 'calc(100% + 8px)' } : undefined}
      initial={{ opacity: 0, x: side === 'left' ? -80 : 80 }}
      animate={isInView ? { opacity: 1, x: 0 } : {}}
      transition={{ duration: 0.55, ease: 'easeOut', delay: 0.5 }}
    >
      {isInView && (
        <div style={side === 'right' ? { transform: 'scaleX(-1)', width: '100%', height: '100%' } : undefined}>
          {children}
        </div>
      )}
    </motion.div>
...
const UBCNeural = ({ side = 'left' }) => (
  <AnimatedStrip side={side} className="ubc-neural-strip">
    <svg className="neural-svg" viewBox="0 0 60 90" aria-hidden="true">
...
const BoeingPlane = ({ side = 'left' }) => (
  <AnimatedStrip side={side} className="boeing-plane-strip">
    <>
...
const TUMGears = ({ side = 'left' }) => (
  <AnimatedStrip side={side} className="tum-gears-strip">
...
const TUMServer = ({ side = 'left' }) => (
  <AnimatedStrip side={side} className="tum-server-strip">
...
const UBCChart = ({ side = 'left' }) => (
  <AnimatedStrip side={side} className="ubc-chart-strip">
...
const ExperienceItem = ({ experience, onOpenModal }) => {
  ...
  const stripConfigs = {
    boeing: { Cmp: BoeingPlane,  wrapper: 'boeing-card-wrapper' },
    ubc2:   { Cmp: UBCNeural,   wrapper: 'ubc-card-wrapper' },
    tum3:   { Cmp: TUMGears,    wrapper: 'tum-gears-card-wrapper' },
    tum4:   { Cmp: TUMServer,   wrapper: 'tum-server-card-wrapper' },
    ubc5:   { Cmp: UBCChart,    wrapper: 'ubc-chart-card-wrapper' },
  };

  const key =
    experience.company === 'Boeing' ? 'boeing' :
    experience.company === 'University of British Columbia' && experience.id === 2 ? 'ubc2' :
    experience.company === 'Technical University of Munich (TUM)' && experience.id === 3 ? 'tum3' :
    experience.company === 'Technical University of Munich (TUM)' && experience.id === 4 ? 'tum4' :
    experience.company === 'University of British Columbia' && experience.id === 5 ? 'ubc5' :
    null;

  if (key) {
    const { Cmp, wrapper } = stripConfigs[key];
    return (
...
const Experience = () => {
  ...
  const openModal = (experience, tab) => setModalState({ experience, tab });
      ...
      <div className="experience-list">
        {experiences.map((exp, index) => (
          <FadeIn key={exp.id} delay={index * 0.1} direction="up">
            <ExperienceItem experience={exp} onOpenModal={openModal} />
          </FadeIn>
        ))}
      </div>
